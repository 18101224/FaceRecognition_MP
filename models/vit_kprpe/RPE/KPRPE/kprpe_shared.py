...
from ..rpe_impl import get_rpe_index_function
...
class iRPE(nn.Module):
    ...
    def __init__(self, head_dim, num_heads=8,
        ...
        self.rpe_index_function = get_rpe_index_function()
    ...
    def _get_rp_bucket(self, x, height=None, width=None):
        ...
        if self.rpe_index_function is not None and self.mode == 'contextual' and self.transposed:
        ...
        if self.mode == 'contextual' and self.transposed:
            if self.rpe_index_function is None:
    ...
    def forward_rpe_transpose(self, x, rp_bucket):
        ...
        if self.mode == 'bias':
            ...
            """
            if self.rpe_index_function is not None:
                return self.rpe_index_function.apply(x, rp_bucket)
            else:
