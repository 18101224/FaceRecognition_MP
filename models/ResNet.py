...
__all__ = ['resnet32', 'resnet50', 'resnext50', 'get_resnet']
...
class ResNet_s(nn.Module):
    ...
    def __init__(self, block, num_blocks, num_classes=10, use_norm=False):
        ...
        if use_norm:
            self.linear = NormedLinear(64 * factor, num_classes)
        else:
            self.linear = nn.Linear(64 * factor, num_classes)
...
def resnet32(num_class, use_norm):
    return ResNet_s(BasicBlock_s, [5,5,5], num_class, use_norm=use_norm)
...
def get_resnet(architecture, pretrained=False):
    if architecture == 'resnet50':
        ...
        return resnext50(pretrained=pretrained)
    else:
