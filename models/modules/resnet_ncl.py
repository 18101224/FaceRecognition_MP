...
__all__ = ['Combiner', 'multi_network', 'multi_network_MOCO']
...
class Combiner:
    def __init__(self, args, num_class_list=None):
        ...
        if self.args.dataset in ['cifar100', 'cifar10', 'places']:
    ...
    def initilize_all_parameters(self):
        ...
        if self.args.dataset in ['cifar100', 'cifar10']:
            self.show_step = 100
            self.CON_ratio = 1.0    
    ...
    def multi_network_default_CON(self, model, criterion, image, label):
        ...
        loss_ce = criterion(output_ce, label, feature=feature, classifier=model.classifier) # NCL loss, 
...
class Cos_Classifier(nn.Module):
    ...
    def get_kernel(self):
        '''
        returns : dim, num_classes
        '''
        return torch.nn.functional.normalize(self.weight, dim=1, p=2).T
...
class multi_Network_MOCO(nn.Module):
    def __init__(self, args, mode="train", num_classes=1000):
        ...
        self.MOCO = nn.ModuleList(
            ...
            for i in range(self.network_num)) # queue 
        ...
        self.backbone = nn.ModuleList(
            eval(self.args.net_type)(
                self.args,
                last_layer_stride=2,
            ) for i in range(self.network_num)) # resnets 


        self.module = nn.ModuleList(
            self._get_module()
            for i in range(self.network_num)) # avg pooling 

        
        self.classifier = nn.ModuleList(
            self._get_multi_classifer(True, self.args.scf)
            for i in range(self.network_num)) # classifiers for supervised contrastive learning 
        self.feat = []
        ...
        self.classifier_MA = nn.ModuleList(
            self._get_multi_classifer(True, self.args.scf)
            for i in range(self.network_num)) # classifiers 
        ...
        if self.args.cf == 'FC':
            self.classifier_ce = nn.ModuleList(
                ...
                for i in range(self.network_num))
        elif self.args.cf == 'FCNorm':
            self.classifier_ce = nn.ModuleList(
                FCNorm(self.get_feature_length(), self.num_classes)
                for i in range(self.network_num))
    ...
    def forward(self, input, **kwargs):
        '''
        if feature_flag : input should be ( [k, bs, 3, h ,w] , [k, bs, 3, h ,w] ) out ( [k, 1D_dim] , [k, 1D_dim] )
        if classifier_flag : input should be ( [k, 1D_dim] , [k, 1D_dim] ) out ( )
        each for query, key. 
        '''
    ...
    def extract_feature(self, input_all, **kwargs):
        ...
        feature = []
        for i in range(self.network_num):
            x = (self.backbone[i])(input[i], label=kwargs['label'][i]) # image shaped feature
            x = (self.module[i])(x) # 1D feature 
            x = x.view(x.shape[0], -1) # 1D feature 
            feature.append(x)
    ...
    def get_logits(self, input_all, **kwargs):
        '''
        input q_features, k_features for query, key. 
        outpus logits for classication, logits for moco, logits for key moco [ k, bs, dims ]
        '''
        ...
        for i in range(self.network_num):
            ...
            output = (self.classifier[i])(feature) # self-supervised 
            logits.append(output) 

            output_ce = (self.classifier_ce[i])(feature) # classification 
            logits_ce.append(output_ce)
