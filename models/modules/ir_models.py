...
def load_pretrained(model_name, config_path, ckpt_root):
    if model_name not in ['ir100_web4m','ir100_web12m','ir100_ms1mv3']:
        raise ValueError('not a correct model name', model_name)
    with open('ir_dict.yaml', 'r') as f:
        ir_dict = yaml.load(f, Loader=yaml.FullLoader)
    config = ir_dict[model_name]
    ckpt_path = os.path.join(ckpt_root, config['ckpt_name'])
    ckpt = torch.load(ckpt_path,map_location='cpu',weights_only=False)['state_dict']
    ckpt = {k[6:]:v for k,v in ckpt.items() if 'model' in k }
...
class Backbone(Module):
    ...
    def forward(self, x, feature=False, all_feature=False):
        ...
        return output if not (all_feature or feature) else (output, features[-1] if not all_feature else output, features)
