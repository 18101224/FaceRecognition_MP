...
CUDA_VISIBLE_DEVICES=0,1,2,3 torchrun --nproc_per_node=4 --rdzv-backend=c10d --rdzv-endpoint=localhost:29501 --nnodes=1 pretrain.py --world_size=4 \
--learning_rate=0.00005 --batch_size=256 \
--dataset_name=AffectNet --dataset_path=../data/AffectNet7 \
--wandb_token=../wandb.txt --kp_rpe_cfg_path=checkpoint/adaface_vit_base_kprpe_webface12m \
--n_epochs=200  \
--LDAM=True --ldam_weight=0.1 \
