...
for LR in "${LEARNING_RATES[@]}"; do
  ...
  CUDA_VISIBLE_DEVICES=0 make_cmd "$LR" "--loss=BCL_ECE --ce_weight=2 --cl_weight=0.6 --ece_weight=0.3  --cosine_scaling=32  \
                          --temperature=0.1 --scheduler=warmup" &  

  CUDA_VISIBLE_DEVICES=1 make_cmd "$LR" "--loss=BCL_ECE --ce_weight=2 --cl_weight=0.6 --ece_weight=0.5  --cosine_scaling=32  \
                          --temperature=0.1 --scheduler=warmup" &  
                          
  CUDA_VISIBLE_DEVICES=2 make_cmd "$LR" "--loss=BCL_ECE --ce_weight=2 --cl_weight=0.6 --ece_weight=1  --cosine_scaling=32  \
                          --temperature=0.1 --scheduler=warmup" &  
